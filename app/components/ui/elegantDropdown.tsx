...
const ElegantDropdown: React.FC<ElegantDropdownProps> = ({
  ...
  return (
    <div className={`relative group ${className}`} ref={dropdownRef}>
      ...
      {/* Dropdown Menu */}
      <div className={`absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-2xl rounded-xl shadow-2xl border border-white/30 overflow-hidden transition-all duration-500 z-50 ${
        isOpen 
        ...
        <div className="py-2 max-h-60 overflow-y-auto">
          {options.map((option, index) => (
            ...
            >
              ...
              <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 w-1 h-0 bg-gradient-to-b from-gray-600 to-gray-800 transition-all duration-300 ${
                selectedOption?.value === option.value ? 'h-full' : 'group-hover/item:h-1/2'
              }`} />
...
export const RoomsDropdown: React.FC<Omit<ElegantDropdownProps, 'options' | 'icon'>> = (props) => {
  const roomOptions = [
...
export const GuestsDropdown: React.FC<Omit<ElegantDropdownProps, 'options' | 'icon'>> = (props) => {
  const guestOptions = [
