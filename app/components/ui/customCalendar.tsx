...
const CustomCalendar: React.FC<CustomCalendarProps> = ({
  ...
  const handleMouseEnter = (date: Date) => {
    if (selectedRange.from && !selectedRange.to && !isPastDate(date) && date >= selectedRange.from) {
      setTempToDate(date);
    }
  };
  ...
  const isDateSelected = (date: Date) => {
    ...
    const fromMatch = selectedRange.from && 
      checkDate.getTime() === new Date(selectedRange.from.getFullYear(), selectedRange.from.getMonth(), selectedRange.from.getDate()).getTime();
    
    const toMatch = selectedRange.to && 
      checkDate.getTime() === new Date(selectedRange.to.getFullYear(), selectedRange.to.getMonth(), selectedRange.to.getDate()).getTime();
    
    const tempToMatch = tempToDate && 
      checkDate.getTime() === new Date(tempToDate.getFullYear(), tempToDate.getMonth(), tempToDate.getDate()).getTime();
          ...
          <div className="mt-6 p-4 bg-gray-50/60 backdrop-blur-sm rounded-xl border border-gray-200/60">
            ...
            <div className="font-medium text-gray-900">
              ...
              {(selectedRange.to || tempToDate) && (
                <>
                  {' → '}
                  {(selectedRange.to || tempToDate)?.toLocaleDateString('es-ES', { 
                    day: 'numeric', 
                    month: 'long',
                    year: 'numeric'
                  })}
