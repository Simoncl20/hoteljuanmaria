...
import { BookOpen, CreditCard, Calendar, Clock, Leaf, Scale, Shield, Edit } from 'lucide-react';
...
const TermsConditionsPage: React.FC = () => {
  ...
  const getIconForSection = (title: string) => {
    if (title.includes("Reservas") || title.includes("Pagos")) return <CreditCard className="w-6 h-6" />;
    if (title.includes("Cancelación")) return <Calendar className="w-6 h-6" />;
    if (title.includes("Check-in") || title.includes("Check-out")) return <Clock className="w-6 h-6" />;
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black flex items-center justify-center">
        <div className="relative bg-white/10 backdrop-blur-2xl border border-white/20 shadow-2xl rounded-xl p-8">
          <div className="animate-pulse flex items-center gap-4">
            ...
            <span className="text-white/70 font-sans text-lg">Cargando términos y condiciones...</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute top-2/3 left-1/5 w-32 h-32 bg-white/6 rounded-full blur-3xl animate-pulse" style={{animationDelay: '1.5s'}} />
        <div className="absolute bottom-1/3 right-1/3 w-28 h-28 bg-white/3 rounded-full blur-3xl animate-pulse" style={{animationDelay: '3s'}} />
      </div>
        ...
        <div className="text-center mb-16 transform transition-all duration-1000 translate-y-0 opacity-100">
          ...
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-xl border border-white/20 rounded-lg px-4 py-2 text-white/80">
            ...
            <span className="font-sans text-sm font-medium">
              Última actualización: {new Date(termsData.lastUpdated).toLocaleDateString('es-ES', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
              })}
          ...
          {highlights.map((highlight, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 opacity-0 hover:opacity-100 transition-opacity duration-700">
                ...
                <div className="absolute bottom-3 left-4 w-3 h-0.5 bg-gradient-to-r from-transparent via-white/20 to-transparent animate-pulse" style={{animationDelay: '0.4s'}}/>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="font-sans text-sm text-white/90 leading-relaxed">
                  Hotel Juan María opera bajo estricto cumplimiento de toda la legislación colombiana vigente, incluyendo normas de sostenibilidad turística, protección del menor, y normatividad ambiental. Nuestro compromiso es brindar servicios seguros, legales y responsables.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          ...
          <button className="relative font-semibold rounded-lg overflow-hidden transition-all duration-700 group px-8 py-4 border border-gray-200/60 text-gray-700 hover:border-gray-300 bg-white/70 backdrop-blur-xl">
            ...
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700">
              ...
              <div className="absolute bottom-1 left-3 w-2 h-0.5 bg-gradient-to-r from-transparent via-gray-300/20 to-transparent animate-pulse" style={{animationDelay: '0.3s'}}/>
            </div>
          </button>
        </div>
      </div>
